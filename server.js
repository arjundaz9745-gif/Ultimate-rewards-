...
const staffWebOnline = new Map(); // userId -> lastSeen ms
let discordClient = null;
let discordBotReady = false;
let discordBotError = null;
...
app.get('/api/me', async (req, res) => {
  ...
  if (staff) {
    staffWebOnline.set(String(req.session.user.id), {
      lastSeen: Date.now(),
...
app.get('/api/staff/online', async (req, res) => {
  const presenceEnabled = !!(discordBotReady && discordClient);
  const staff = [];
  const now = Date.now();
  for (const id of STAFF_USER_IDS) {
    const sid = String(id);
    const cached = staffPresence.get(sid);
    ...
    staff.push({
      id: sid,
      name: cached?.name || web?.name || u.global_name || u.username || 'Staff',
      avatar: cached?.avatar || web?.avatar || u.avatar || null,
      status
    });
  }
  ...
  try {
    ...
    async function refreshStaffPresence() {
      try {
        ...
        if (!guild) {
          console.error('Presence: bot is not in any guild / bad GUILD_ID');
          return;
        }
        ...
        for (const id of STAFF_USER_IDS) {
          const sid = String(id);
          try {
            const member = await guild.members.fetch({ user: sid, force: true });
            const status = member.presence?.status || 'offline';
            staffPresence.set(sid, {
            ...
            try {
              ...
              if (r.ok) {
                ...
                staffPresence.set(sid, {
                  name: m.nick || m.user?.global_name || m.user?.username || 'Staff',
                  avatar: m.user?.avatar || null,
                  status: 'offline'
                });
              } else {
                staffPresence.set(sid, { name: 'Staff', avatar: null, status: 'offline' });
              }
            } catch {
              staffPresence.set(sid, { name: 'Staff', avatar: null, status: 'offline' });
            }
        ...
        console.log('Presence refreshed for', staffPresence.size, 'staff');
      } catch (e) {
        console.error('Presence refresh:', e.message);
        discordBotError = e.message;
      }
    }
    ...
    client.once('ready', async () => {
      ...
      setInterval(refreshStaffPresence, 45_000);
    });
